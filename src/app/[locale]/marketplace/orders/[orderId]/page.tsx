...
export default function OrderTrackingPage({ params }: PageProps) {
  ...
  const orderDocRef = useMemo(() => {
    return firestore ? doc(firestore, 'orders', orderId) : null;
  }, [firestore, orderId]);
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 space-y-6">
      <Link href="/marketplace/orders" className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground mb-2 transition-colors">
        <ArrowLeft className="mr-2 h-4 w-4" /> Back to My Orders
      </Link>
        ...
        <Card className="md:col-span-3 overflow-hidden shadow-sm">
          ...
          <CardContent className="pt-8 pb-10">
            {isCancelled ? (
              ...
              <div className="relative flex justify-between items-start w-full max-w-3xl mx-auto px-2 md:px-4">
                {/* Stepper Line Background */}
                <div className="absolute left-6 right-6 md:left-8 md:right-8 top-4 md:top-6 h-0.5 md:h-1 bg-muted -translate-y-1/2 z-0" />
                
                {/* Stepper Line Fill */}
                <div 
                  className="absolute left-6 md:left-8 top-4 md:top-6 h-0.5 md:h-1 bg-primary -translate-y-1/2 z-0 transition-all duration-500 ease-in-out"
                  style={{ 
                    right: `calc(100% - 12px - ${(currentStep / 4) * 100}% - 6px)`
                  }}
                />
                ...
                {steps.map((step, idx) => {
                  ...
                  return (
                    <div key={idx} className="relative flex flex-col items-center z-10 w-16 md:w-20">
                      <div 
                        className={`w-8 h-8 md:w-12 md:h-12 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
                          isCompleted 
                            ? 'bg-emerald-500 border-emerald-500 text-white shadow-emerald-200/50 shadow-md' 
                            : isActive 
                            ? 'bg-background border-primary text-primary shadow-primary-200 shadow-lg ring-4 ring-primary/20 scale-115' 
                      ...
                      <span className={`text-[9px] md:text-xs font-semibold mt-2 md:mt-3 text-center truncate w-full ${isActive ? 'text-primary font-bold' : isCompleted ? 'text-foreground' : 'text-muted-foreground'}`}>
                        {step.label}
                      </span>
                    </div>
                  );
