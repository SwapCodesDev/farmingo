...
export function CommentThread({ comment, allComments, postId, postAuthorId, isPostAuthor, pinnedCommentId, commentAction, voteAction, pinAction, depth = 0, collectionName = 'posts' }: CommentThreadProps) {
  const { user } = useUser();
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSubmitting(true);
    try {
        await updateComment(postId, comment.id, editText, collectionName);
        setIsEditing(false);
  ...
  const handleDelete = async () => {
    try {
        await deleteComment(postId, comment.id, collectionName);
        ...
        <div className={cn("p-3 rounded-lg", depth % 2 === 0 ? "bg-muted/50" : "bg-muted/25", isPinned && "border-2 border-primary/50")}>
                            ...
                            <DropdownMenuPortal>
                                <DropdownMenuSubContent className="max-h-64 overflow-y-auto">
                                    {availableLanguages.map(lang => (
                                        <DropdownMenuItem key={lang.value} onClick={() => handleTranslate(lang.label)}>
                                            {lang.label}
                                        </DropdownMenuItem>
                                    ))}
                                </DropdownMenuSubContent>
          ...
          <CollapsibleContent>
            {isEditing ? (
                ...
                <>
                <p className="text-sm mt-2 whitespace-pre-wrap">{isTranslating ? 'Translating...' : displayedText}</p>
                {translatedText && (
                ...
                <CommentVoteControl postId={postId} comment={comment} voteAction={(v) => voteAction(postId, comment.id, v)} />
                <Button
                    variant="ghost"
                    size="sm"
