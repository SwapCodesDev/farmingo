...
export function VoteControl({ post, voteAction, orientation = 'horizontal' }: VoteControlProps) {
  ...
  return (
    <div className={cn("flex items-center gap-1 bg-muted/50 rounded-full",
      ...
      <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full" onClick={() => handleVote('up')} disabled={!user || votingType !== null}>
        {votingType === 'up' ? (
      ...
      <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full" onClick={() => handleVote('down')} disabled={!user || votingType !== null}>
        {votingType === 'down' ? (
