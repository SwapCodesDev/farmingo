...
import type { MarketplacePost, Comment } from '@/types';
...
export function MarketplacePostDetailClient({ postId }: MarketplacePostDetailClientProps) {
  ...
  const pinnedComment = useMemo(() => {
    ...
    return comments.find((c: any) => c.id === post.pinnedCommentId);
  }, [post, comments]);

  const topLevelComments = useMemo(() => {
    if (!comments) return [];
    return comments.filter((comment: any) => !comment.parentId && comment.id !== post?.pinnedCommentId);
  }, [comments, post]);
