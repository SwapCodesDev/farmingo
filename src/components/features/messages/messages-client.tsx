...
export function MessagesClient({ currentUser }: MessagesClientProps) {
  ...
  const sortedAndFilteredConversations = useMemo(() => {
    ...
    const filtered = conversations.filter((conv: ConversationDoc) => {
    ...
    return filtered.sort((a: ConversationDoc, b: ConversationDoc) => {
    ...
    <Card className="overflow-hidden h-full">
      <div className="p-4 border-b">
        ...
        <div className="relative mt-4">
          ...
          <Input placeholder={t('search-placeholder')} className="pl-9" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      </div>
