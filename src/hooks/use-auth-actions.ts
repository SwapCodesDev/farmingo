...
export function useAuthActions() {
  ...
  const createPost = useCallback(
    async (postData: PostData) => {
      ...
      return createPostAction(firestore, user, postData);
    },
  ...
  const updatePost = useCallback(
    async (postId: string, postData: Partial<PostData>) => {
      ...
      return updatePostAction(firestore, postId, postData);
    },
    [user, firestore, toast]
  );

  const addComment = useCallback(
    async (postId: string, text: string, parentId: string | null = null, collectionName: string = 'posts') => {
      if (!user || !firestore) {
        toast({
          variant: 'destructive',
          title: 'Not authenticated',
      ...
      return addCommentAction(firestore, user, postId, text, parentId, collectionName);
    },
    [user, firestore, toast]
  );

  const updateComment = useCallback(
    async (postId: string, commentId: string, text: string, collectionName: string = 'posts') => {
      if (!user || !firestore) {
        toast({
          variant: 'destructive',
          title: 'Not authenticated',
      ...
      return updateCommentAction(firestore, postId, commentId, text, collectionName);
    },
    [user, firestore, toast]
  );

  const deleteComment = useCallback(
    async (postId: string, commentId: string, collectionName: string = 'posts') => {
      if (!user || !firestore) {
        toast({
          variant: 'destructive',
          title: 'Not authenticated',
      ...
      return deleteCommentAction(firestore, postId, commentId, collectionName);
    },
    [user, firestore, toast]
  );
  ...
  const voteOnPost = useCallback(
    async (postId: string, vote: 'up' | 'down', collectionName: string = 'posts') => {
      if (!user || !firestore) {
        toast({
          variant: 'destructive',
          title: 'Not authenticated',
      ...
      return voteOnPostAction(firestore, user.uid, postId, vote, collectionName);
    },
    [user, firestore, toast]
  );

  const voteOnComment = useCallback(
    async (postId: string, commentId: string, vote: 'up' | 'down', collectionName: string = 'posts') => {
        if (!user || !firestore) {
            toast({
                variant: 'destructive',
                title: 'Not authenticated',
        ...
        return voteOnCommentAction(firestore, user.uid, postId, commentId, vote, collectionName);
    },
    [user, firestore, toast]
  );

  const pinComment = useCallback(
    async (postId: string, commentId: string | null, collectionName: string = 'posts') => {
      if (!user || !firestore) {
        toast({
          variant: 'destructive',
          title: 'Not authenticated',
      ...
      return pinCommentAction(firestore, postId, commentId, collectionName);
    }, [user, firestore, toast]
  );
  ...
  const deletePost = useCallback(
    async (postId: string) => {
        ...
        return deletePostAction(firestore, postId);
    },
    [user, firestore, toast]
);
