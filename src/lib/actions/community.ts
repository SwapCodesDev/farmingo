...
export async function createPost(
  ...
  try {
    ...
    if (serverError && serverError.code === 'permission-denied') {
...
export function updatePost(
  ...
  updateDoc(postRef, updatedData).catch(async (serverError: any) => {
    if (serverError && serverError.code === 'permission-denied') {
        const permissionError = new FirestorePermissionError({
        path: postRef.path,
        operation: 'update',
...
export async function addComment(
  ...
  const commentsCollection = collection(firestore, collectionName, postId, 'comments');
  ...
  const postRef = doc(firestore, collectionName, postId);
...
export function updateComment(
    ...
    updateDoc(commentRef, updateData).catch(async (serverError: any) => {
        if (serverError && serverError.code === 'permission-denied') {
            const permissionError = new FirestorePermissionError({
...
export function deleteComment(
    ...
    runTransaction(firestore, async (transaction) => {
        ...
        if (!commentData.parentId) {
            ...
            const parentCommentRef = doc(firestore, collectionName, postId, 'comments', commentData.parentId);
...
export function pinComment(
  ...
  const postRef = doc(firestore, collectionName, postId);
  updateDoc(postRef, { pinnedCommentId: commentId }).catch((serverError: any) => {
    if (serverError && serverError.code === 'permission-denied') {
        const permissionError = new FirestorePermissionError({
            path: postRef.path,
            operation: 'update',
...
export function deletePost(firestore: Firestore, postId: string) {
  const postRef = doc(firestore, 'posts', postId);
  deleteDoc(postRef).catch(async (serverError: any) => {
    if (serverError && serverError.code === 'permission-denied') {
        const permissionError = new FirestorePermissionError({
        path: postRef.path,
        operation: 'delete',
